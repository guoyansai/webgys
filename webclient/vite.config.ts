...
export default defineConfig(({ mode }) => {
  ...
  let outputNames: any = {};
  // 从环境变量中加载配置
  const { VITE_HTTP_HOST, VITE_HTTP_PORT, VITE_HTTP_API, VITE_DIST_DIR, VITE_API_URL, VITE_WEB_PATH, VITE_OBFUSCATE_ENABLE } = loadEnv(mode, process.cwd(), '');
    ...
    ...(!IS_DEV && IS_OBFUSCATE_ENABLED
      ? [
          vitePluginBundleObfuscator({
            ...
            excludes: ['vendor-lua', 'vendor-3d', 'vendor-flow', 'vendor-main', 'vendor-qrcode', 'vendor-pinyin'],
            options: {
              compact: true,
              target: 'browser',
